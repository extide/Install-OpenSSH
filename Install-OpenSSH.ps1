...
if ($IsAdmin) {
    Write-Output "Script is running elevated." -Forecolor Green
}
else {
...
Write-Host "Downloading Archive" -Forecolor Green
Invoke-WebRequest -Uri $OpenSSHURL -OutFile $GitZipName
Write-Host "Download Complete, now expanding and copying to destination" -Forecolor Green
...
#Do Install
Write-Host "Running Install Commands" -Forecolor Green
...
If (!(Test-Path $env:ProgramData\ssh)) {
    Write-Host "Creating ProgramData\ssh directory" -Forecolor Green
    New-Item -ItemType Directory -Force -Path $env:ProgramData\ssh | Out-Null
}

#Setup sshd_config
Write-Host "Configure server config file" -Forecolor Green
...
If (!(Test-Path "~\.ssh")) {
    Write-Host "Creating User .ssh directory" -Forecolor Green
    New-Item -ItemType Directory -Force -Path "~\.ssh" | Out-Null
}

#Set ssh_config
Write-Host "Configure client config file" -Forecolor Green
Add-Content -Path ~\.ssh\config -Value "GSSAPIAuthentication yes"

#Start the service
Write-Host "Starting Service" -Forecolor Green
Start-Service sshd
...
if ($existingPath -notmatch $InstallPath.Replace("\", "\\")) {
    Write-Host "Adding OpenSSH Directory to path" -Forecolor Green
    $newpath = "$existingPath;$InstallPath"
    Set-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager\Environment' -Name PATH -Value $newPath
}

#Make sure user keys are configured correctly
Write-Host "Ensuring HostKey file permissions are correct" -Forecolor Green
...
Write-Host "Ensuring UserKey file permissions are correct" -Forecolor Green
powershell.exe -ExecutionPolicy Bypass -Command '. .\FixUserFilePermissions.ps1 -Confirm:$false'

#Add firewall rule
Write-Host "Creating firewall rule" -Forecolor Green
New-NetFirewallRule -Name sshd -DisplayName 'OpenSSH Server (sshd)' -Enabled True -Direction Inbound -Protocol TCP -Action Allow -LocalPort 22

#Set Shell to powershell
Write-Host "Setting default shell to powershell" -Forecolor Green
